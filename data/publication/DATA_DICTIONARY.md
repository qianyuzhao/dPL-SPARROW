...
| `02_model_parameters_by_HUC12_year.csv` | 116,360 | Annual learned N export, stream-loss, and reservoir-loss parameters plus derived attenuation. |
...
| `05_scenario_N_export_rate_by_HUC12.csv` | 5,818 | Baseline and sequential counterfactual N-export-rate scenarios. Scenario labels identify inputs held at early-period values. |
...
| `08_SHAP_values_N_export_rate.csv` | 20,000 | Sample identifiers, raw feature values, and SHAP values for the learned N-export-rate parameter. |
...
The CSVs were generated by `scripts/generate_publication_data.py`. The script
records the source artifact for each table in its section comments.
